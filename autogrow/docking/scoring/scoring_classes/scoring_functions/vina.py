...
class VINA(ParentScoring):
    ...
    def find_files_to_score(self, file_path):
        ...
        list_of_files = glob.glob(file_path + "docked*.pdbqt")
        list_of_files =[file for file in list_of_files if "ligand" not in file]
        print("the files to score should be fucking pinted here")
        print(list_of_files)
    ...
    def get_score_from_a_file(self, file_path):
        ...
        """
        print("we are in ge scores::::" + file_path)
               ...
               if "Affinity" in line:
                    ...
                    if affinity_complex is None:
                        ...
                        if affinity_complex > float(line_split[0]):
                            affinity_complex = float(line_split[0])
                    print("affinity_complex 1 " + str(affinity_complex))
        ...
        lig_info = [ligand_short_name, basefile_strip, sc]
        f = open("/home/abdou/autgorw/complex/abdoutxt" , "a")
        print(str(lig_info))
        f.write(str(lig_info))
        f.close()
