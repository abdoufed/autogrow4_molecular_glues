...
class VinaDocking(ParentDocking):
    ...
    def dock_ligand(self, lig_pdbqt_filename):
        ...
        new_file_name = os.path.split(lig_pdbqt_filename)[0] + "/docked_"+os.path.split(lig_pdbqt_filename)[1]
        torun = (
            "{} {} {}".format(
        ...
        torun = "/home/abdou/autgorw/complex/autogrow/docking/docking_executables/vina/autodock_vina_1_1_2_linux_x86/bin/vina_split --input " +new_file_name
        results = self.execute_docking_vina(torun)
        interest = new_file_name.split(".pdb")[0]+"_ligand_1.pdbqt"
        


        torun = (
            "{} {} {}".format(
                timeout_option, docking_timeout_limit, "/home/abdou/autgorw/complex/autogrow/docking/docking_executables/vina/autodock_vina_1_1_2_linux_x86/bin/vina "
            )
            + " --score_only --center_x "
            + str(vars["center_x"])
            + " --center_y "
            ...
            + str(vars["size_z"])
            + " --receptor "
            + os.path.split(self.receptor_pdbqt_file)[0]+"/complex.pdbqt"
            + " --ligand "
            + interest
        ...
        torun = (
            torun
            + " >>"
            + os.path.split(lig_pdbqt_filename)[0]+"/complex_docked_"+os.path.split(lig_pdbqt_filename)[1]
    ...
    def check_docked(self, pdb_file):
        ...
        if not os.path.exists(os.path.split(pdb_file)[0]+"/docked_" +os.path.split(pdb_file)[1]+ "qt"):
